...
function buildTaskFilter(fieldsMap: Record<string, string>, app: string, scene: string, datePreset: string) {
  ...
  if (dateField) {
    const day = resolveFilterDate(datePreset);
    if (day) {
      const ms = resolveExactDateMs(day);
      if (ms) {
        conds.push({ field_name: dateField, operator: "is", value: ["ExactDate", ms] });
      } else {
        conds.push({ field_name: dateField, operator: "is", value: [day] });
      }
    }
  }
...
async function loadExistingBookIDs(baseURL: string, token: string, taskRef: BitableRef, fieldsMap: Record<string, string>, opts: any, pageSize: number, useView: boolean) {
  ...
  const targetDay = resolveFilterDate(opts.date || "Today");
...
async function main() {
  ...
  program
    ...
    const sourceURL = (opts.bitableUrl || opts.bitable_url || "").trim();
    if (!sourceURL) {
  ...
  sourceFieldOptions(
    program
      ...
      .option("--source-url <url>", "Source Bitable URL (optional, for tracking)")
    ...
    const taskURL = (opts.task_url || Env("TASK_BITABLE_URL", "")).trim();
    if (!taskURL) {
    ...
    const records = derived
      .filter((item) => !opts.skip_existing || !existingBookIDs.has(item.book_id))
      .map((item) => buildTaskFields(fieldsMap, { ...item, date: createDate }))
      .filter((fields) => Object.keys(fields).length)
      .map((fields) => ({ fields }));
    if (!records.length) {
      logger.info("result", { data: { derived: derived.length, filtered, created: 0 } });
      process.exit(0);
    }
    let created = 0;
    ...
    logger.info("result", { data: { derived: derived.length, filtered, created, failed: errors.length, errors, elapsed_seconds: elapsed } });
  ...
  sourceFieldOptions(
    ...
    const useView = false;
    const pageSize = 200;
    const sourceItems = await fetchAllRecords(baseURL, token, sourceRef, pageSize, "", false);
    ...
    const records = derived
      ...
      .map((fields) => ({ fields }));
    if (!records.length) {
      logger.info("result", { data: { source_total: sourceItems.length, filtered, created: 0 } });
      process.exit(0);
    }
    let created = 0;
    const start = Date.now();
    ...
    const elapsed = Math.floor(((Date.now() - start) / 1000) * 1000) / 1000;
    logger.info("result", { data: { source_total: sourceItems.length, filtered, derived: derived.length, created, failed: errors.length, errors, elapsed_seconds: elapsed } });
    process.exit(errors.length ? 1 : 0);
  });
